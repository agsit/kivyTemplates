...
#
# Description:
#   Module with generic methods that are typically usefull for a GUI.
#
# Standard imports
import os
import platform
...
class userPath(object):
  """
    Description:
      ...
      2) opens a file dialog to allow the user to select a file or folder.  At the same
         ...
         The purpose of this tracking is to avoid for the user to manually
         browse back to a given location to re-select the next file or select
         the same file at a later time.
  ...
  def __init__(self, appModule=None): # Object constructor
    ...
    if self.appModule is not None:
      ...
      # dict 'appCfg'. This 'appCfg' dictionary keeps track of the last
      # location that the user used.
      # The following guarantees that the initialization file is  available and has some
      # default values.
      # 1. Figure out the complete module path (the command below will retrieve the
      #    full path of the App module)
