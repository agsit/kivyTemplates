...
class baseApp(App):
  """
    ...
    The Kivy class is defined in the baseApp.kv Kivy file (NOTE: Kivy filename must be ALL lower case and not include the "App" postfix). 
  ...
  def build(self):
    ...
    """
    self.title = 'baseAppGUI (ver0.0r210214)'
    ...
    self.fileIO = userPath('baseApp.py')
    # Instantantiate a baseAppGUI object for the GUI and at the same time also send the
    # reference to this main app object to the baseAppGUI object so that properties from both
    ...
    return baseAppGUI(self)


  def selectFile(self):
    self.fileIO.fileType = [('text files', ('.txt', '.text')), ('all files', '.*')]
    self.fileIO.pathSelect(pathTag='baseAppGUI')
...
#======================================== GUI class ===============================================#
class baseAppGUI(GridLayout):
  """
  This will instantiate the "root" Kivy property and the name used here MUST match exactly the Kivy
  class name used in the baseApp.kv file and the name used in the build method of the app class
  defined above.
  """
